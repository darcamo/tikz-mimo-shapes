...
\begin{tikzpicture}[every node/.append style={scale=1}]
  ...
  \tikzset{bs sizes/.style={minimum width=1cm, minimum height=4cm}}
  \tikzset{ue sizes/.style={minimum width=1.2cm, minimum height=2cm}}
  ...
  \draw (BS1Pos) node[draw,bs sizes,dashed, gray] {};
  ...
  % xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  \path (4,1.5) coordinate (UE1Pos);

  \draw (UE1Pos) node[UserTerminal,draw,ue sizes,fill=gray, button fill color=gray!40] (myterminal) {UE};
    ...
    center}
  {
    ...
    % \draw[fill=green] (myterminal.\ancora) circle (1pt) node[scale=0.8,red!50!black,rotate=45] {\tiny{\ancora}};
  }
