...
\begin{tikzpicture}[every node/.append style={scale=1}]
  \draw[help lines] (-0.5,-7.5) grid (10,7.5);
  ...
  \path (8.5,0.0) coordinate (mimo2biggerpos);

  % xxxxxxxxxx TRY CHANGING THE VALUES HERE TO SEE THE xxxxxxxxxxxxxxxxxxxx
  \tikzset{test mimo node/.style={fill=blue!30,draw,mimotwo,thick,
...
% Nodes with one antenna
  ...
  \node[test mimo node,mimoone,right=3cm of mimo1txnode,left antennas] (mimo1rxnode) {\pgftypesetversion};

  % Nodes with two antennas
  \draw (mimo2pos) node[test mimo node] (mimo2txnode) {\pgftypesetversion};
  \node[test mimo node,right=3cm of mimo2txnode,left antennas] (mimo2rxnode) {\pgftypesetversion};

  % Nodes with tree antennas
  \draw (mimo3pos) node[test mimo node,mimothree] (mimo3txnode) {\pgftypesetversion};
  \node[test mimo node,right=3cm of mimo3txnode,mimothree,left antennas] (mimo3rxnode) {\pgftypesetversion};

  % Nodes with many antennas
  \draw (mimoindpos)  node[draw,test mimo node,mimoind] (mimoindtxnode) {\pgftypesetversion};
  \draw node[right=3cm of mimoindtxnode,draw,test mimo node,mimoind,left antennas] (mimoindrxnode) {\pgftypesetversion};
  % xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  % xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
