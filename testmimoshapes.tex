...
\begin{tikzpicture}
  ...
  \draw (0,0) node[draw,mimotwo] (mimodois) {rectangle};

  \draw (0,-1cm) node[draw,outer sep=0.5\pgflinewidth] (retangulo) {rectangle};
  \draw node[draw,mimotwo,left antennas,left=0mm of mimodois] {mais um};
  \draw node[draw,left=10mm of retangulo] (maisUm) {mais um};

  %\draw[fill=red] (retangulo.west) circle (1pt);
  \draw (maisUm) -- (retangulo);
